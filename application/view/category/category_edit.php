...
if (isset($this->categoryBO) && $this->categoryBO != NULL) {
    ...
    <h1>
        <?php echo USER_TITLE_EDIT_PROFILE_OF . " " . CATEGORY_TITLE_CATEGORY; ?> "<strong><?php
            if (isset($this->categoryBO->name)) {
                echo $this->categoryBO->name;
            }

            ?></strong>"
        <a class="page-title-action" ajaxlink="<?php echo URL . CATEGORY_CP_ADD_NEW; ?>" ajaxtarget=".wrap" href="#" onclick="openAjaxLink(this)" ><?php echo TITLE_ADD_NEW; ?></a>
    </h1>
    ...
    <form id="form-your-profile" novalidate="novalidate"  method="POST" enctype="multipart/form-data" action="<?php echo URL . CATEGORY_CP_EDIT; ?>">
        <input type="hidden" value="update" name="action">
        <input type="hidden" value="<?php
        if (isset($this->categoryBO->term_taxonomy_id)) {
            echo htmlspecialchars($this->categoryBO->term_taxonomy_id);
        }
            ...
            <tbody>
                ...
                <tr class="category-parent-wrap"><th><label for="parent"><?php echo CATEGORY_TITLE_PERENT; ?> <span style="color: red;" class="description"><?php echo TITLE_REQUIRED; ?></span></label></th>
                    <td>
                        <select id="parent" name="parent" >
                            ...
                            ?>
                            <option value="0" <?php if ($this->categoryBO->parent == 0 || $this->categoryBO->parent == "0") { ?>
                                        selected="selected"
                                    <?php } ?> ><?php echo TITLE_NONE; ?></option>    
                        </select>
                        <p class="description"><?php echo CATEGORY_DESC_PERENT; ?></p>
                    </td>
                </tr>
                ...
                <tr class="images-wrap">
                    ...
                    <td>
                        ...
                        <p class="images"  style="float: left; width: 100%; margin-bottom: 30px;"><?php echo CATEGORY_DESC_IMAGES; ?></p>
        ...
        <p class="submit"><input type="submit" value="<?php echo CATEGORY_TITLE_UPDATE_CATEGORY; ?>" class="button button-primary" id="submit" name="submit"></p>
    </form>
    <script>
        ...
        jQuery("#form-your-profile").submit(function (e) {
            e.preventDefault();
            if (!validateFormEditCategory()) {
                return;
            }
            var name = jQuery('#form-your-profile input[name="name"]').val();
            if (confirm('<?php echo CATEGORY_CONFIRM_EDIT_INFO_CATEGORY; ?>' + name + '<?php echo CONFIRM_EDIT_INFO_CANCEL_OK; ?>')) {
